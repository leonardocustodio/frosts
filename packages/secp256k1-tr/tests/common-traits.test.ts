/**
 * Common traits tests for FROST secp256k1-SHA256-TR (Taproot).
 * Ported from frost-secp256k1-tr/tests/common_traits_tests.rs
 *
...
import { createSecureRng, SIGNATURE_LENGTH } from "./helpers/index.js";
import { element1, element2, scalar1, proofOfKnowledge, bytesToHex } from "./helpers/samples.js";
import {
  Secp256K1Sha256TR,
  // Use *Impl imports for classes with static methods
  SigningKeyImpl as SigningKey,
  SigningNoncesImpl as SigningNonces,
  SigningCommitmentsImpl as SigningCommitments,
  NonceCommitmentImpl as NonceCommitment,
  SignatureShareImpl as SignatureShare,
  SecretShareImpl as SecretShare,
  KeyPackageImpl as KeyPackage,
  PublicKeyPackageImpl as PublicKeyPackage,
  SigningShareImpl as SigningShare,
  VerifyingShareImpl as VerifyingShare,
  CoefficientCommitmentImpl as CoefficientCommitment,
  VerifiableSecretSharingCommitmentImpl as VerifiableSecretSharingCommitment,
...
const ciphersuite = Secp256K1Sha256TR;
 ...
 * For Taproot, proof of knowledge is BIP-340 format (64 bytes: x-only R + scalar).
 */
function createSampleRound1Package() {
  const commitment = createSampleCommitment();
  const pokBytes = proofOfKnowledge();
  // BIP-340 format: first 32 bytes is x-only R, next 32 is z
  // We need to convert x-only R to SEC1 compressed (prepend 0x02)
  const xOnlyR = pokBytes.slice(0, 32);
  const rBytes = new Uint8Array(33);
  rBytes[0] = 0x02; // Even y prefix
  rBytes.set(xOnlyR, 1);
  const R = ciphersuite.deserializeElement(rBytes);
...
describe("FROST secp256k1-SHA256-TR Common Traits Tests", () => {
...
describe("Taproot-specific Tests", () => {
  it("should have correct BIP-340 signature length", () => {
    // Taproot uses 64-byte signatures (x-only R + scalar)

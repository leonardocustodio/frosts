...
import {
  ...
  secretShare,
  keyPackage,
  publicKeyPackage as _publicKeyPackage,
  publicKeyPackageNew,
  round1Package,
  round2Package,
...
describe("FROST secp256k1-SHA256-TR JSON Serialization Tests", () => {
  ...
  describe("Round1 Package JSON Serialization", () => {
    it("should serialize round1::Package to JSON", () => {
      ...
      const json = round1PackageToJson(pkg);
      const decoded = round1PackageFromJson(json);

      // Verify roundtrip equality
      expect(decoded.commitment.coefficients.length).toBe(pkg.commitment.coefficients.length);
      // Proof of knowledge has R and z components
      expect(json.proof_of_knowledge.length).toBe(SIGNATURE_LENGTH * 2); // 64 bytes = 128 hex chars
    });
  ...
  describe("Round2 Package JSON Serialization", () => {
    it("should serialize round2::Package to JSON", () => {
      ...
      const json = round2PackageToJson(pkg);

...
class Secp256K1Sha256Impl implements RandomizedCiphersuite {
  ...
  computeBindingFactorList<C extends Ciphersuite>(
    ...
    const vkBytes = (verifyingKey as VerifyingKey<Secp256K1Sha256Impl>).serialize();
  ...
  computeGroupCommitment<C extends Ciphersuite>(
    ...
    for (const [identifier, commitment] of signingPackage.signingCommitments) {
      ...
      const scaledBinding = this.elementMul(bindingElement, bindingFactor.toScalar() as Secp256K1Scalar);
...
function aggregateImpl(
  ...
  const groupCommitment = Secp256K1Sha256.computeGroupCommitment(
    signingPackage,
    bindingFactorList,
  );
  ...
  for (const [identifier, share] of signatureShares) {
    ...
    if (verifySigShares) {
      ...
      const challengeTimesLambda = Secp256K1Sha256.scalarMul(
        challenge.toScalar(),
        lambdaI,
      );
      const verifyingElement = verifyingShare.toElement();
      const scaledVerifyingShare = Secp256K1Sha256.elementMul(verifyingElement, challengeTimesLambda);

...
describe("VerifiableSecretSharingCommitment (Secp256K1)", () => {
  let rng: CryptoRng;
  ...
  describe("Serialization", () => {
    it("should serialize VSS commitment correctly", () => {
      tests.checkSerializeVssCommitment(Secp256K1Sha256, rng);
    });

    it("should serialize whole VSS commitment correctly", () => {
      tests.checkSerializeWholeVssCommitment(Secp256K1Sha256, rng);
    });
  });

  describe("Deserialization", () => {
    it("should deserialize VSS commitment correctly", () => {
      tests.checkDeserializeVssCommitment(Secp256K1Sha256, rng);
    });

    it("should deserialize whole VSS commitment correctly", () => {
      tests.checkDeserializeWholeVssCommitment(Secp256K1Sha256, rng);
    });
  });

  describe("Deserialization Errors", () => {
    it("should fail to deserialize with invalid element", () => {
      tests.checkDeserializeVssCommitmentError(Secp256K1Sha256, rng, elementsJson);
    });

    it("should fail to deserialize whole with invalid element", () => {
      tests.checkDeserializeWholeVssCommitmentError(Secp256K1Sha256, rng, elementsJson);
    });

    it("should fail to deserialize whole with invalid length", () => {
      tests.checkDeserializeWholeVssCommitmentInvalidLength(Secp256K1Sha256, rng);
    });
  });

  describe("Public Key Package Computation", () => {
    it("should compute public key package from list of commitments", async () => {
      await tests.checkComputePublicKeyPackage(Secp256K1Sha256, rng);
    });
  });
});

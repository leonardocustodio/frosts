/**
 * Common traits tests for FROST secp256k1-SHA256.
 * Ported from frost-secp256k1/tests/common_traits_tests.rs
 *
...
const ciphersuite = Secp256K1Sha256;
...
function createSampleRound1Package() {
  ...
  // Parse proof of knowledge: first 33 bytes is R (SEC1 compressed), next 32 is z
  const R = ciphersuite.deserializeElement(pokBytes.slice(0, 33));
  const z = ciphersuite.deserializeScalar(pokBytes.slice(33, 65));
  const pok = { R, z };
...
describe("FROST secp256k1-SHA256 Common Traits Tests", () => {

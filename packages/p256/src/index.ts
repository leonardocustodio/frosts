...
// Use Point from @noble/curves (renamed from ProjectivePoint in v2.0.0)
const ProjectivePoint = p256.Point;
...
class P256Sha256Impl implements RandomizedCiphersuite {
   ...
   */
  challenge(R: P256Point, verifyingKey: unknown, message: Uint8Array): CoreChallenge<this> {
...
function aggregateImpl(
  ...
  const groupCommitment = P256Sha256.computeGroupCommitment(signingPackage, bindingFactorList);
  ...
  for (const [identifier, share] of signatureShares) {
    ...
    if (verifySigShares) {
      ...
      const challengeTimesLambda = P256Sha256.scalarMul(challenge.toScalar(), lambdaI);

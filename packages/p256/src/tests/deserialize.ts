/**
 * Deserialization tests for FROST-P256-SHA256.
 * Ported from frost-p256/src/tests/deserialize.rs
 *
 * These tests verify proper handling of deserialization edge cases
 * specific to the P-256 curve, including:
 * - Non-canonical point encodings
 * - Identity element rejection
 * - Zero identifier rejection
 *
 * Note: P-256 uses SEC1 compressed point format (33 bytes).
...
describe("P256-SHA256 Deserialization", () => {
  describe("Non-canonical encodings", () => {
    ...
    it("should reject non-canonical x-coordinate (x >= p)", () => {
      ...
      // The P-256 field prime p = 0xFFFFFFFF00000001000000000000000000000000FFFFFFFFFFFFFFFFFFFFFFFF
      // This test uses x = p + 5 = 0xFFFFFFFF00000001000000000000000000000001000000000000000000000004
      ...
      //
      // try {
      //   P256Sha256.group.deserialize(nonCanonicalPoint);
      //   expect.fail("Should have thrown GroupError");
      // } catch (e) {
      //   expect(e).toBeInstanceOf(GroupError);
      //   expect((e as GroupError).type).toBe(GroupErrorType.MalformedElement);
  ...
  describe("Identity element", () => {
    it("should reject the identity element (point at infinity)", () => {
      ...
      // Once P256Sha256 ciphersuite is implemented:
      // const encodedIdentity = new Uint8Array(P256_ELEMENT_SIZE);
      // encodedIdentity.fill(0);
      //
      // try {
      //   P256Sha256.group.deserialize(encodedIdentity);
      //   expect.fail("Should have thrown GroupError");
      // } catch (e) {
      //   expect(e).toBeInstanceOf(GroupError);
      //   expect((e as GroupError).type).toBe(GroupErrorType.MalformedElement);
  ...
  describe("Zero identifier", () => {
    it("should reject deserializing identifier 0", () => {
      ...
      // Once P256Sha256 ciphersuite is implemented:
      // const zeroIdentifier = new Uint8Array(P256_SCALAR_SIZE);
...
export { P256_SCALAR_SIZE, P256_ELEMENT_SIZE, hexToBytes };

...
describe("Rerandomized FROST", () => {
  ...
  describe("Deprecated API Coverage (Intentionally Omitted)", () => {
    it("documents check_from_randomizer_and_signing_package omission", () => {
      // INTENTIONALLY OMITTED: check_from_randomizer_and_signing_package
      //
      // The Rust test `check_from_randomizer_and_signing_package` tests the deprecated
      // `Randomizer::from_randomizer_and_signing_package()` private method which:
      //
      // 1. Is marked as PRIVATE (not pub fn) in Rust
      // 2. Is only used by the deprecated `Randomizer::new()` method
      // 3. Requires serializing the entire SigningPackage
      // 4. Hashes: rng_randomizer || serialized_signing_package
      //
      // The TypeScript implementation deliberately does NOT implement this deprecated
      // API path. Instead, we implement only the recommended non-deprecated API:
      //
      // - Randomizer.newFromCommitments() - generates randomizer from commitments
      // - Randomizer.regenerateFromSeedAndCommitments() - regenerates from seed
      //
      // The new API improves security by:
      // 1. Binding the randomizer to commitments (not the message)
      // 2. Using a seed-based approach for reproducibility
      // 3. Allowing participants to verify the randomizer without trusting the coordinator
      //
      // The key properties tested by check_from_randomizer_and_signing_package are:
      // - Determinism (same inputs -> same output) - TESTED via checkFromSeedAndSigningCommitments
      // - Different RNG -> different randomizers - TESTED via checkFromSeedAndSigningCommitments
      // - Different messages -> different randomizers - N/A (message not in new API)
      //
      // See: https://github.com/ZcashFoundation/frost/blob/main/frost-rerandomized/src/lib.rs
      expect(true).toBe(true);
    });

    it("documents deprecated Randomizer.new() omission", () => {
      // INTENTIONALLY OMITTED: Randomizer::new() (deprecated)
      //
      // The Rust API has a deprecated `Randomizer::new(rng, signing_package)` method
      // that creates a randomizer by hashing an RNG output with the serialized
      // SigningPackage. This is deprecated in favor of `new_from_commitments()`.
      //
      // TypeScript implements only the recommended API:
      // - Randomizer.newFromCommitments(ciphersuite, rng, signingCommitments)
      //
      // The deprecated test `check_from_randomizer` in Rust uses this API path,
      // but our TypeScript implementation tests the equivalent functionality
      // using the non-deprecated newFromCommitments approach.
      expect(true).toBe(true);
    });

    it("documents deprecated RandomizedParams.new() omission", () => {
      // INTENTIONALLY OMITTED: RandomizedParams::new() (deprecated)
      //
      // Similar to Randomizer::new(), there's a deprecated
      // `RandomizedParams::new(verifying_key, signing_package, rng)` method
      // that creates params using the deprecated Randomizer::new() internally.
      //
      // TypeScript implements only the recommended API:
      // - RandomizedParams.newFromCommitments()
      // - RandomizedParams.regenerateFromSeedAndCommitments()
      // - RandomizedParams.fromRandomizer()
      expect(true).toBe(true);
    });
  });
});

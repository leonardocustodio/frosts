/**
 * Common traits tests for FROST Ed448-SHAKE256.
 * Ported from frost-ed448/tests/common_traits_tests.rs
 *
...
const ciphersuite = Ed448Shake256;
...
function createSampleRound1Package() {
  ...
  // Parse proof of knowledge: first 57 bytes is R, next 57 is z (Ed448 is 57-byte elements/scalars)
  const R = ciphersuite.deserializeElement(pokBytes.slice(0, 57));
  const z = ciphersuite.deserializeScalar(pokBytes.slice(57, 114));
  const pok = { R, z };
...
describe("FROST Ed448-SHAKE256 Common Traits Tests", () => {

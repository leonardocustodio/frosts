/**
 * Re-randomized FROST tests for Ed448-SHAKE256.
 * Ported from frost-ed448/tests/rerandomized_tests.rs
 *
...
describe("FROST Ed448-SHAKE256 Re-randomized Tests", () => {
  let rng: CryptoRng;
  ...
  describe("Randomizer Properties", () => {
    it("should create randomizer from commitments", () => {
      ...
      const commitment = new SigningCommitments(Ed448Shake256, hidingCommitment, bindingCommitment);

      const identifier = Identifier.fromU16(Ed448Shake256, 1);
      ...
      expect(seed.length).toBe(57); // Ed448 scalar size
    });
    ...
    it("should produce different randomizers with different rng", () => {
      ...
      const [randomizer1] = Randomizer.newFromCommitments(Ed448Shake256, rng1, signingCommitments);

      const [randomizer2] = Randomizer.newFromCommitments(Ed448Shake256, rng2, signingCommitments);
    ...
    it("should serialize and deserialize randomizer", () => {
      ...
      const [randomizer] = Randomizer.newFromCommitments(Ed448Shake256, rng, signingCommitments);

      const serialized = randomizer.serialize();
      expect(serialized.length).toBe(57); // Ed448 scalar size

      const deserialized = Randomizer.deserialize(Ed448Shake256, serialized);
  ...
  describe("RandomizedParams", () => {
    it("should create params from randomizer", () => {
      ...
      const commitmentsData = signingCommitmentsData();
      const verifyingKeyElement = Ed448Shake256.deserializeElement(commitmentsData.hiding);
      const verifyingKey = VerifyingKey.create(Ed448Shake256, verifyingKeyElement);
  ...
  describe("Re-randomized Signing Flow", async () => {
    it("should complete re-randomized signing flow with dealer", async () => {
      ...
      const keyPackages = new Map<string, KeyPackage<typeof Ed448Shake256>>();
      ...
      const nonces = new Map<string, SigningNonces<typeof Ed448Shake256>>();
      ...
      const signingPackage: SigningPackage<typeof Ed448Shake256> = {
      ...
      const signatureShares = new Map<
        Identifier<typeof Ed448Shake256>,
        SignatureShare<typeof Ed448Shake256>
      >();
  ...
  describe("Unlinkability", () => {
    it("should produce different randomized keys for same group key", async () => {
      ...
      for (const idHex of participantIds) {
        ...
        const [, signingCommitments] = commit(Ed448Shake256, keyPackage.signingShare, rng);
        commitmentsMap.set(keyPackage.identifier, signingCommitments);
      }

...
function isIdentityPoint(point: Ed448Point): boolean {
  ...
  // But ed448.ExtendedPoint.ZERO.toRawBytes() gives us the proper encoding
  try {
    const decoded = ed448.ExtendedPoint.fromHex(point);
    return decoded.equals(ed448.ExtendedPoint.ZERO);
  } catch {
    return false;
  }
}
...
function pointAdd(a: Ed448Point, b: Ed448Point): Ed448Point {
  try {
    const aPoint = ed448.ExtendedPoint.fromHex(a);
    const bPoint = ed448.ExtendedPoint.fromHex(b);
...
function pointMul(point: Ed448Point, scalar: Ed448Scalar): Ed448Point {
  try {
    const p = ed448.ExtendedPoint.fromHex(point);
...
function scalarBaseMul(scalar: Ed448Scalar): Ed448Point {
  try {
    const s = bytesToBigInt(scalar);
    const result = ed448.ExtendedPoint.BASE.multiply(s);
    return result.toRawBytes();
  } catch (_e) {
    throw GroupError.malformedElement();
  }
...
function getGenerator(): Ed448Point {
  return ed448.ExtendedPoint.BASE.toRawBytes();
}
...
function getIdentity(): Ed448Point {
  return ed448.ExtendedPoint.ZERO.toRawBytes();
}
...
function deserializePoint(bytes: Uint8Array): Ed448Point {
  ...
  try {
    const point = ed448.ExtendedPoint.fromHex(bytes);

    // Check for identity element
    if (point.equals(ed448.ExtendedPoint.ZERO)) {
      throw GroupError.invalidIdentityElement();
    }
...
class Ed448Shake256Impl implements RandomizedCiphersuite {
  ...
  H2(m: Uint8Array): Ed448Scalar {
    const prefix = new Uint8Array([
      0x53, 0x69, 0x67, 0x45, 0x64, 0x34, 0x34, 0x38, // "SigEd448"
      0x00, 0x00, // two null bytes for empty context
    ]);
  ...
  computeBindingFactorList<C extends Ciphersuite>(
    ...
    const vkBytes = (verifyingKey as VerifyingKey<Ed448Shake256Impl>).serialize();
  ...
  computeGroupCommitment<C extends Ciphersuite>(
    ...
    for (const [identifier, commitment] of signingPackage.signingCommitments) {
      ...
      const scaledBinding = this.elementMul(bindingElement, bindingFactor.toScalar() as Ed448Scalar);
...
function aggregateImpl(
  ...
  const groupCommitment = Ed448Shake256.computeGroupCommitment(
    signingPackage,
    bindingFactorList,
  );
  ...
  for (const [identifier, share] of signatureShares) {
    ...
    if (verifySigShares) {
      ...
      const challengeTimesLambda = Ed448Shake256.scalarMul(
        challenge.toScalar(),
        lambdaI,
      );

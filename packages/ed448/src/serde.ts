/**
 * JSON serialization (serde) support for FROST Ed448-SHAKE256.
 *
...
export function signingCommitmentsToJson(
  commitments: SigningCommitments<Ed448Shake256Impl>,
 ...
 */
export function signingCommitmentsFromJson(json: unknown): SigningCommitments<Ed448Shake256Impl> {
  ...
  const hiding = NonceCommitment.deserialize(Ed448Shake256, hidingBytes);
  const binding = NonceCommitment.deserialize(Ed448Shake256, bindingBytes);

  return new SigningCommitments(Ed448Shake256, hiding, binding);
}
 ...
 */
export function signingPackageToJson(pkg: SigningPackage<Ed448Shake256Impl>): SigningPackageJson {
 ...
 */
export function signingPackageFromJson(json: unknown): SigningPackage<Ed448Shake256Impl> {
  if (typeof json !== "object" || json === null) {
    throw new Error("Invalid JSON: expected object");
  }
  const obj = json as Record<string, unknown>;
  ...
  const commitmentsMap = new Map<
    Identifier<Ed448Shake256Impl>,
    SigningCommitments<Ed448Shake256Impl>
  >();
  ...
  for (const [idHex, commitmentJson] of Object.entries(commitmentsObj)) {
    ...
    const id = Identifier.deserialize(Ed448Shake256, idBytes);
  ...
  return new SigningPackageImpl(Ed448Shake256, commitmentsMap, message);
}
 ...
 */
export function signatureShareToJson(share: SignatureShare<Ed448Shake256Impl>): SignatureShareJson {
  return {
    header: { ...DEFAULT_HEADER },
 ...
 */
export function signatureShareFromJson(json: unknown): SignatureShare<Ed448Shake256Impl> {
  if (typeof json !== "object" || json === null) {
    throw new Error("Invalid JSON: expected object");
  }
  const obj = json as Record<string, unknown>;
  ...
  return SignatureShare.deserialize(Ed448Shake256, shareBytes);
}
 ...
 */
export function secretShareToJson(share: SecretShare<Ed448Shake256Impl>): SecretShareJson {
  return {
    header: { ...DEFAULT_HEADER },
 ...
 */
export function secretShareFromJson(json: unknown): SecretShare<Ed448Shake256Impl> {
  if (typeof json !== "object" || json === null) {
    throw new Error("Invalid JSON: expected object");
  }
  const obj = json as Record<string, unknown>;
  ...
  const signingShare = SigningShare.deserialize(Ed448Shake256, signingShareBytes);

  const commitmentHexes = obj["commitment"] as string[];
  const commitmentBytes = commitmentHexes.map(hexToBytes);
  const commitment = VerifiableSecretSharingCommitment.deserialize(Ed448Shake256, commitmentBytes);

  return new SecretShare(Ed448Shake256, id, signingShare, commitment);
}
 ...
 */
export function keyPackageToJson(pkg: KeyPackage<Ed448Shake256Impl>): KeyPackageJson {
  return {
    header: { ...DEFAULT_HEADER },
    ...
    verifying_key: bytesToHex(Ed448Shake256.serializeElement(pkg.verifyingKey)),
 ...
 */
export function keyPackageFromJson(json: unknown): KeyPackage<Ed448Shake256Impl> {
  if (typeof json !== "object" || json === null) {
    throw new Error("Invalid JSON: expected object");
  }
  const obj = json as Record<string, unknown>;
  ...
  const verifyingShare = VerifyingShare.deserialize(Ed448Shake256, verifyingShareBytes);

  const verifyingKeyBytes = hexToBytes(obj["verifying_key"] as string);
  const verifyingKey = Ed448Shake256.deserializeElement(verifyingKeyBytes);
...
export function publicKeyPackageToJson(
  pkg: PublicKeyPackage<Ed448Shake256Impl>,
 ...
 */
export function publicKeyPackageFromJson(json: unknown): PublicKeyPackage<Ed448Shake256Impl> {
  if (typeof json !== "object" || json === null) {
    throw new Error("Invalid JSON: expected object");
  }
  const obj = json as Record<string, unknown>;
  ...
  const verifyingShares = new Map<string, VerifyingShare<Ed448Shake256Impl>>();

  for (const [idHex, shareHex] of Object.entries(verifyingSharesObj)) {
    // Validate the identifier is not all zeros
    const idBytes = hexToBytes(idHex);
    Identifier.deserialize(Ed448Shake256, idBytes); // This will throw if invalid (all zeros)
  ...
  return new PublicKeyPackage(Ed448Shake256, verifyingShares, verifyingKey, minSigners);
}
 ...
 */
export function round1PackageToJson(pkg: round1.Package<Ed448Shake256Impl>): Round1PackageJson {
  // Serialize the proof of knowledge (DkgSignature) as R || z
  const pokR = Ed448Shake256.serializeElement(pkg.proofOfKnowledge.R);
  const pokZ = Ed448Shake256.serializeScalar(pkg.proofOfKnowledge.z);
 ...
 */
export function round1PackageFromJson(json: unknown): round1.Package<Ed448Shake256Impl> {
  if (typeof json !== "object" || json === null) {
    throw new Error("Invalid JSON: expected object");
  }
  const obj = json as Record<string, unknown>;
  ...
  const scalarSize = Ed448Shake256.scalarSize();
  ...
  return {
    ...
    serialize(): Uint8Array[] {
      ...
      const pokR = Ed448Shake256.serializeElement(proofOfKnowledge.R);
      const pokZ = Ed448Shake256.serializeScalar(proofOfKnowledge.z);
  ...
  } as unknown as round1.Package<Ed448Shake256Impl>;
}
 ...
 */
export function round2PackageToJson(pkg: round2.Package<Ed448Shake256Impl>): Round2PackageJson {
  return {
    header: { ...DEFAULT_HEADER },
    signing_share: bytesToHex(pkg.signingShare.serialize()),
  };
 ...
 */
export function round2PackageFromJson(json: unknown): round2.Package<Ed448Shake256Impl> {
  if (typeof json !== "object" || json === null) {
    throw new Error("Invalid JSON: expected object");
  }
  const obj = json as Record<string, unknown>;
  ...
  } as unknown as round2.Package<Ed448Shake256Impl>;
}

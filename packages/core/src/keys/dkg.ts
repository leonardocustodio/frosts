...
export async function part3<C extends Ciphersuite>(
  ...
  // Call postDkg if the ciphersuite defines it (e.g., for taproot tweaking)
  if (ciphersuite.postDkg !== undefined) {
    const [tweakedKeyPackage, tweakedPublicKeyPackage] = ciphersuite.postDkg(
      keyPackage,
      publicKeyPackage,
    );
    return [tweakedKeyPackage as KeyPackage<C>, tweakedPublicKeyPackage as PublicKeyPackage<C>];
  }

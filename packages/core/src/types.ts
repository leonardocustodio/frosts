...
import { bytesToHex } from "./serialization.js";
...
export interface IdentifierLike<C extends Ciphersuite> {
  toScalar(): C["Scalar"];
...
export interface Ciphersuite {
   ...
   * Defaults to elementSize() + scalarSize() if not specified.
   * For BIP-340/Taproot ciphersuites this is typically 64 (2 * scalarSize).
   */
  ...
  /**
   * Optional. Post-process DKG output (called at the end of part3).
   * Used for ciphersuites that need to apply transformations after DKG,
   * such as taproot tweaking for secp256k1-tr.
   */
...
// These are exported as they appear in public method signatures
export interface SigningNoncesLike<C extends Ciphersuite> {
  readonly hiding: { toScalar(): C["Scalar"] };
  readonly binding: { toScalar(): C["Scalar"] };
  readonly commitments: SigningCommitmentsLike<C>;
}

export interface SigningCommitmentsLike<C extends Ciphersuite> {
  readonly hiding: { toElement(): C["Element"] };
  readonly binding: { toElement(): C["Element"] };
  equals(other: SigningCommitmentsLike<C>): boolean;
}

export interface KeyPackageLike<C extends Ciphersuite> {
...
export interface PublicKeyPackageLike<C extends Ciphersuite> {
  readonly verifyingShares: Map<string, { toElement(): C["Element"] }>;
  readonly verifyingKey: C["Element"];
  readonly minSigners: number | undefined;
}

export interface SignatureShareLike<C extends Ciphersuite> {
  toScalar(): C["Scalar"];
}

export interface GroupCommitmentShareLike<C extends Ciphersuite> {
  toElement(): C["Element"];
}

export interface VerifyingShareLike<C extends Ciphersuite> {
  toElement(): C["Element"];
}
...
export interface SigningPackage<C extends Ciphersuite> {
   ...
   */
  readonly signingCommitments: Map<IdentifierLike<C>, SigningCommitments<C>>;
 ...
 */
export class SigningPackageImpl<C extends Ciphersuite> implements SigningPackage<C> {
  /** The ciphersuite instance */
  readonly ciphersuite: C;
   ...
   * @param signingCommitments - Map of identifier to signing commitments
   * @param message - The message to sign
   * @returns A new SigningPackage instance
   */
  static create<C extends Ciphersuite>(
    ciphersuite: C,
    signingCommitments: Map<IdentifierLike<C>, SigningCommitments<C>>,
    message: Uint8Array,
  ): SigningPackageImpl<C> {
   ...
   *
   * @param other - The other SigningPackage
   * @returns True if equal
   */
  equals(other: SigningPackageImpl<C>): boolean {
    // Compare messages
    if (this.message.length !== other.message.length) return false;
    for (let i = 0; i < this.message.length; i++) {
      if (this.message[i] !== other.message[i]) return false;
    }

    // Compare commitments maps
    if (this.signingCommitments.size !== other.signingCommitments.size) return false;

    // Convert to string keys for comparison
    const thisMap = new Map<string, SigningCommitments<C>>();
    for (const [id, commitment] of this.signingCommitments) {
      thisMap.set(bytesToHex(id.serialize()), commitment);
    }

    for (const [id, commitment] of other.signingCommitments) {
      const key = bytesToHex(id.serialize());
      const thisCommitment = thisMap.get(key);
      if (thisCommitment === undefined || thisCommitment.equals(commitment) === false) {
        return false;
      }
    }
  ...
  /**
   * Clone this SigningPackage.
   *
   * @returns A new SigningPackage with the same values
   */
  clone(): SigningPackageImpl<C> {
    const newCommitments = new Map<IdentifierLike<C>, SigningCommitments<C>>();
    for (const [id, commitment] of this.signingCommitments) {
      newCommitments.set(id.clone(), commitment.clone());
    }
    return new SigningPackageImpl(this.ciphersuite, newCommitments, new Uint8Array(this.message));
  }
   ...
   */
  toString(): string {
    const msgHex = bytesToHex(this.message);
    const truncatedMsg = msgHex.length > 16 ? `${msgHex.slice(0, 16)}...` : msgHex;
    return `SigningPackage { commitments: ${this.signingCommitments.size}, message: ${truncatedMsg} }`;
  }
}
...
// bytesToHex is imported from ./serialization.js

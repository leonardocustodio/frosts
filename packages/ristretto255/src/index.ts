...
// Use Point from ristretto255 (renamed from RistrettoPoint in v2.0.0)
...
function isIdentityPoint(point: Ristretto255Point): boolean {
  try {
    const decoded = RistrettoPoint.fromBytes(point);
...
function pointAdd(a: Ristretto255Point, b: Ristretto255Point): Ristretto255Point {
  try {
    const aPoint = RistrettoPoint.fromBytes(a);
    const bPoint = RistrettoPoint.fromBytes(b);
...
function pointMul(point: Ristretto255Point, scalar: Ristretto255Scalar): Ristretto255Point {
  try {
    const p = RistrettoPoint.fromBytes(point);
...
function getGenerator(): Ristretto255Point {
  return RistrettoPoint.BASE.toBytes();
}
...
class Ristretto255Sha512Impl implements RandomizedCiphersuite {
   ...
   */
  challenge(R: Ristretto255Point, verifyingKey: unknown, message: Uint8Array): CoreChallenge<this> {
  ...
  computeGroupCommitment<C extends Ciphersuite>(
    ...
    for (const [identifier, commitment] of signingPackage.signingCommitments) {
      ...
      const scaledBinding = this.elementMul(
        bindingElement,
        bindingFactor.toScalar() as Ristretto255Scalar,
      );
...
function aggregateImpl(
  ...
  for (const [identifier, share] of signatureShares) {
    ...
    if (verifySigShares) {
      ...
      const challengeTimesLambda = Ristretto255Sha512.scalarMul(challenge.toScalar(), lambdaI);
      const verifyingElement = verifyingShare.toElement();
      const scaledVerifyingShare = Ristretto255Sha512.elementMul(
        verifyingElement,

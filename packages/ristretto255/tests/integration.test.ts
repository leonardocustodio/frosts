...
describe("FROST Ristretto255-SHA512 Integration Tests", () => {
  ...
  describe("Repairable Threshold Scheme", () => {
    it("should recover a lost share using RTS", async () => {
      await tests.checkRts(Ristretto255Sha512, rng);
    });

    it("should verify repair_share_step_1 math", async () => {
      await tests.checkRepairShareStep1(Ristretto255Sha512, rng);
    });

    it("should fail repair share step 1 with invalid min_signers", async () => {
      await tests.checkRepairShareStep1FailsWithInvalidMinSigners(Ristretto255Sha512, rng);
    });
  });

  describe("Share Refresh", () => {
    it("should refresh shares with dealer", async () => {
      await tests.checkRefreshSharesWithDealer(Ristretto255Sha512, rng);
    });

    it("should serialize and deserialize refresh data correctly", async () => {
      await tests.checkRefreshSharesWithDealerSerialisation(Ristretto255Sha512, rng);
    });
    ...
    it("should fail refresh with invalid identifier", async () => {
      await tests.checkRefreshSharesWithDealerFailsWithInvalidIdentifier(Ristretto255Sha512, rng);
    });

    it("should refresh shares with DKG", async () => {
      await tests.checkRefreshSharesWithDkg(Ristretto255Sha512, rng);
    });

    it("should refresh shares with DKG using smaller threshold", async () => {
      await tests.checkRefreshSharesWithDkgSmallerThreshold(Ristretto255Sha512, rng);
    });
  });
  ...
  describe("Test Vectors", () => {
    it("should sign with test vectors", async () => {
      ...
      await tests.checkSignWithTestVectors(Ristretto255Sha512, vectorsJson.default);
    });
    ...
    it("should perform DKG keygen with test vectors", async () => {
      ...
      await tests.checkDkgKeygen(Ristretto255Sha512, vectorsDkgJson.default);
    });
    ...
    it("should sign with test vectors using big identifiers", async () => {
      ...
      await tests.checkSignWithTestVectors(Ristretto255Sha512, vectorsBigJson.default);
    });
  });
  ...
  describe("Identifier Derivation", () => {
    ...
    it("should generate identifiers from numeric and derived values", () => {
      ...
      const id1 = Identifier.fromU16(Ristretto255Sha512, 1);
      const id2 = Identifier.fromU16(Ristretto255Sha512, 2);
      const id42 = Identifier.fromU16(Ristretto255Sha512, 42);
      const id255 = Identifier.fromU16(Ristretto255Sha512, 255);
      const id256 = Identifier.fromU16(Ristretto255Sha512, 256);
      ...
      const id1Again = Identifier.fromU16(Ristretto255Sha512, 1);
      expect(id1.equals(id1Again)).toBe(true);

      // Derive identifiers from arbitrary bytes
      const derived1 = Identifier.derive(Ristretto255Sha512, new Uint8Array([1, 2, 3, 4]));
      const derived2 = Identifier.derive(Ristretto255Sha512, new Uint8Array([5, 6, 7, 8]));

      // Different input = different output
      expect(derived1.equals(derived2)).toBe(false);

      // Same input = same output
      const derived1Again = Identifier.derive(Ristretto255Sha512, new Uint8Array([1, 2, 3, 4]));
  ...
  describe("Async Signing", () => {
    it("should complete async signing flow with dealer", async () => {
      ...
      await tests.asyncCheckSign(Ristretto255Sha512, rng);
    });
  });
});
...
describe("Test Vector Verification", () => {
  it("should verify test vector signature", () => {
    ...
    tests.checkTestVectorSignature(Ristretto255Sha512, VECTORS);
  });

  it("should verify test vector verifying key derivation", () => {
    // Use the generic test vector verifying key verification from core
    tests.checkTestVectorVerifyingKey(Ristretto255Sha512, VECTORS);
  });
...
describe("Batch Verification", () => {
  ...
  it("should verify a batch of valid signatures", () => {
    tests.checkBatchVerify(Ristretto255Sha512, rng);
  });

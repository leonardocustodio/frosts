/**
 * Re-randomized FROST tests for Ed25519-SHA512.
 * Ported from frost-ed25519/tests/rerandomized_tests.rs
 *
...
describe("FROST Ed25519-SHA512 Re-randomized Tests", () => {
  let rng: CryptoRng;
  ...
  describe("Randomizer Properties", () => {
    it("should create randomizer from commitments", () => {
      ...
      const hidingCommitment = NonceCommitment.deserialize(Ed25519Sha512, commitmentsData.hiding);
      const bindingCommitment = NonceCommitment.deserialize(Ed25519Sha512, commitmentsData.binding);
      const commitment = new SigningCommitments(Ed25519Sha512, hidingCommitment, bindingCommitment);

      const identifier = Identifier.fromU16(Ed25519Sha512, 1);
      const signingCommitments = new Map<
        Identifier<typeof Ed25519Sha512>,
        SigningCommitments<typeof Ed25519Sha512>
      >();
    ...
    it("should produce different randomizers with different rng", () => {
      ...
      const [randomizer1] = Randomizer.newFromCommitments(Ed25519Sha512, rng1, signingCommitments);

      const [randomizer2] = Randomizer.newFromCommitments(Ed25519Sha512, rng2, signingCommitments);
    ...
    it("should serialize and deserialize randomizer", () => {
      ...
      const [randomizer] = Randomizer.newFromCommitments(Ed25519Sha512, rng, signingCommitments);

      const serialized = randomizer.serialize();
      expect(serialized.length).toBe(32); // Scalar size

      const deserialized = Randomizer.deserialize(Ed25519Sha512, serialized);
  ...
  describe("RandomizedParams", () => {
    it("should create params from randomizer", () => {
      ...
      const commitmentsData = signingCommitmentsData();
      const verifyingKeyElement = Ed25519Sha512.deserializeElement(commitmentsData.hiding);
      const verifyingKey = VerifyingKey.create(Ed25519Sha512, verifyingKeyElement);
  ...
  describe("Re-randomized Signing Flow", async () => {
    it("should complete re-randomized signing flow with dealer", async () => {
      ...
      const keyPackages = new Map<string, KeyPackage<typeof Ed25519Sha512>>();
      ...
      const nonces = new Map<string, SigningNonces<typeof Ed25519Sha512>>();
      ...
      const signingPackage: SigningPackage<typeof Ed25519Sha512> = {
      ...
      const signatureShares = new Map<
        Identifier<typeof Ed25519Sha512>,
        SignatureShare<typeof Ed25519Sha512>
      >();
  ...
  describe("Unlinkability", () => {
    it("should produce different randomized keys for same group key", async () => {
      ...
      for (const idHex of participantIds) {
        ...
        const [, signingCommitments] = commit(Ed25519Sha512, keyPackage.signingShare, rng);
        commitmentsMap.set(keyPackage.identifier, signingCommitments);
      }

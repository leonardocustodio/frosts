...
// Access Point from the ed25519 object (renamed from ExtendedPoint in v2.0.0)
...
/**
 * Alias for Ed25519Point for compatibility.
 */
type Ed25519Element = Ed25519Point;
...
function isIdentityPoint(point: Ed25519Point): boolean {
  try {
    const decoded = ExtendedPoint.fromBytes(point);
...
function pointAdd(a: Ed25519Point, b: Ed25519Point): Ed25519Point {
  try {
    const aPoint = ExtendedPoint.fromBytes(a);
    const bPoint = ExtendedPoint.fromBytes(b);
...
function pointMul(point: Ed25519Point, scalar: Ed25519Scalar): Ed25519Point {
  try {
    const p = ExtendedPoint.fromBytes(point);
...
function scalarBaseMulFn(scalar: Ed25519Scalar): Ed25519Point {
  try {
    ...
    if (s === 0n) {
      return ExtendedPoint.ZERO.toBytes();
    }
...
function getGenerator(): Ed25519Point {
  return ExtendedPoint.BASE.toBytes();
}
...
function deserializePoint(bytes: Uint8Array): Ed25519Point {
  ...
  try {
    const point = ExtendedPoint.fromBytes(bytes);
...
class Ed25519Sha512Impl implements RandomizedCiphersuite {
   ...
   */
  challenge(R: Ed25519Point, verifyingKey: unknown, message: Uint8Array): CoreChallenge<this> {
    // Build the preimage: R || verifying_key || message
    const rBytes = this.serializeElement(R);
    // Handle both VerifyingKey objects and raw elements (Ed25519 points)
    ...
    if (
      ...
      pkBytes = this.group.serialize(verifyingKey as Ed25519Element);
    }
  ...
  computeBindingFactorList<C extends Ciphersuite>(
    ...
    if (
      ...
      // It's a raw element, serialize it directly
      vkBytes = this.group.serialize(verifyingKey as Ed25519Element);
    }
  ...
  computeGroupCommitment<C extends Ciphersuite>(
    ...
    for (const [identifier, commitment] of signingPackage.signingCommitments) {
      ...
      const scaledBinding = this.elementMul(
        bindingElement,
        bindingFactor.toScalar() as Ed25519Scalar,
      );
...
// ---------------------------------------------------------------------------
// Serde (JSON serialization) submodule re-export
// ---------------------------------------------------------------------------

// Re-export the serde module
...
function aggregateImpl(
  ...
  const groupCommitment = Ed25519Sha512.computeGroupCommitment(signingPackage, bindingFactorList);
  ...
  for (const [identifier, share] of signatureShares) {
    ...
    if (verifySigShares) {
      ...
      const challengeTimesLambda = Ed25519Sha512.scalarMul(challenge.toScalar(), lambdaI);
